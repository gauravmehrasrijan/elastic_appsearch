...
class ReferenceUIForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    // kint($elastic_appsearch_referenceui->getFieldUrl()); exit;
    $form['label'] = [
      '#type' => 'textfield',
      '#title' => $this->t('Label'),
      '#maxlength' => 255,
      '#default_value' => $elastic_appsearch_referenceui->label(),
      '#description' => $this->t("Label for the Reference ui."),
      '#required' => TRUE,
    ];
    ...
    $form['status'] = [
      ...
      '#description' => $this->t('Status of the reference ui instance.'),
      '#required' => FALSE,
    ];
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Reference ui.', [
          '%label' => $elastic_appsearch_referenceui->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Reference ui.', [
          '%label' => $elastic_appsearch_referenceui->label(),
        ]));
    }
